...
(function(global) {
     ...
     * modify the suite name when it is used inside the sonar report.
    ...
    /**
     * A delegate for letting the consumer
     * modify the report filename when it is used inside the sonar report.
     * This is useful when running a test suite against multiple capabilities
     * because the report can have unique names for each combination of suite/spec
     * and capability/test environment.
     *
    ...
    /**
     * Generates sonar XML for the given spec run. There are various options
     ...
     * Usage:
     *
     * jasmine.getEnv().addReporter(new jasmineReporters.SonarXmlReporter(options));
     *
     ...
     *   xml output file (default: sonarresults-)
     *   NOTE: if consolidateAll is true, the default is simply "sonarresults" and
     *     this becomes the actual filename, ie "sonarresults.xml"
     ...
     *   to an XSLT stylesheet to add to the sonar XML file so that it can be
     ...
     * xml-output elements of the sonar reports {default: false}. If a systemOut delegate is defined and captureStdout
     * is true, the output of the spec can be accessed via spec._stdout
     */
    exportObject.SonarXmlReporter = function(options) {
        var self = this;
        self.started = false;
        self.finished = false;
        // sanitize arguments
        ...
        if (self.consolidateAll) {
            self.filePrefix = options.filePrefix || 'sonarresults';
        } else {
            self.filePrefix = typeof options.filePrefix === 'string' ? options.filePrefix : 'sonarresults-';
        }
        ...
        self.writeFile = function(filename, text) {
            ...
            log("Warning: writing sonar report failed for '" + path + "', '" +
                filename + "'. Reasons:\n" +
                errors.join("\n")
            );
        };
        ...
        function suiteAsXml(suite) {
            var xml = '\n <file path="' + getFullyQualifiedSuiteName(suite) + '">';
        ...
        function specAsXml(spec) {
            ...
            xml += ' duration="' + elapsed(spec._startTime, spec._endTime) + '"';
